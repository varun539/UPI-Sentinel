...
export default function NetworksPage() {
  ...
  /*
   * Load network candidates from the backend.
   *
   * Risk filtering is performed server-side so that
   * Medium and Low candidates are not hidden behind
   * the top overall results.
   */
  useEffect(() => {
    ...
    async function loadNetworks() {
      try {
        ...
        if (riskFilter !== "ALL") {
          params.set("risk_band", riskFilter);
        }

        const response = await fetch(
          `${API}/networks?${params.toString()}`,
          {
            cache: "no-store",
        ...
        if (!response.ok) {
          throw new Error(
            `Network API failed: ${response.status}`
          );
        }
  ...
  async function openNetwork(network: Network) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Network detail API failed: ${response.status}`
        );
      }
      ...
      // Keep the summary card visible if the
  ...
  /*
   * Search is still handled locally because it only
   * searches the currently returned candidate set.
   *
   * Risk filtering is handled by the backend.
   */
  const filteredNetworks = useMemo(() => {
    ...
    return networks.filter((network) => {
      return (
        String(network.component_id)
          .toLowerCase()
          .includes(query) ||
        String(network.risk_band || "")
          .toLowerCase()
          .includes(query) ||
        String(network.signals || "")
          .toLowerCase()
          .includes(query)
      );
    });
  ...
  const highRisk = networks.filter(
    (network) =>
      String(network.risk_band || "").toUpperCase() ===
      "HIGH"
  ).length;
  ...
  const totalChargebacks = networks.reduce(
    (sum, network) =>
      sum +
      (Number(network.total_chargebacks) || 0),
    0
  );
      ...
      <section className="main-content">
        ...
        <section className="stats-grid network-stats">
          ...
          <div className="stat-card">
            ...
            <div className="stat-value">
              {totalChargebacks.toLocaleString(
                "en-IN"
              )}
            </div>
        ...
        <section className="network-hero">
          <div className="network-hero-copy">
            ...
            <h2>
              Map suspicious relationship
              clusters
            </h2>

            <p>
              UPI Sentinel connects transaction
              participants into a graph and
              prioritizes dense or chargeback-heavy
          ...
          <div
            ...
            aria-label="Illustration of users connected to a merchant"
          >
            ...
            <div
              className="graph-node graph-user user-1"
              title="User"
            >
              U
            </div>
            ...
            <div
              className="graph-node graph-user user-2"
              title="User"
            >
              U
            </div>
            ...
            <div
              className="graph-node graph-user user-3"
              title="User"
            >
              U
            </div>
            ...
            <div
              className="graph-node graph-user user-4"
              title="User"
            >
              U
            </div>
            ...
            <div
              className="graph-node graph-user user-5"
              title="User"
            >
              U
            </div>
            ...
            <div
              className="graph-node graph-user user-6"
              title="User"
            >
              U
            </div>

            <div className="graph-label merchant-label">
              MERCHANT
            </div>
        ...
        <section className="panel network-panel">
          <div className="panel-header">
            ...
            <div className="queue-description">
              Candidates ranked by network risk
              score.
            </div>
          </div>
          ...
          {!loading && !error && (
            <div className="network-filter-status">
              Showing{" "}
              <strong>
                {filteredNetworks.length.toLocaleString(
                  "en-IN"
                )}
              </strong>{" "}
              {riskFilter === "ALL"
                ? "highest-risk candidates"
                : `${riskFilter.toLowerCase()}-risk candidates`}
            </div>
          )}
          ...
          {!loading &&
            ...
            filteredNetworks.length === 0 && (
              <div className="empty-state">
                No network candidates match the
                current filters.
              </div>
            )}
                      ...
                      <div className="network-card-top">
                        <div>
                          <div className="network-id">
                            NETWORK #
                            {network.component_id}
                          </div>

                          <div className="network-title">
                            {network.user_count} users
                            <span>↔</span>
                            {network.merchant_count}{" "}
                            merchants
                          </div>
                        </div>
        ...
        >
          ...
          >
            ...
            <div className="drawer-section">
              ...
              <div className="relationship-box">
                <div>
                  ...
                  <strong>
                    {scoreValue(
                      selected.avg_relationship_risk
                    )}
                  </strong>
                </div>
                ...
                <div>
                  ...
                  <strong>
                    {scoreValue(
                      selected.max_relationship_risk
                    )}
                  </strong>
                </div>
                ...
                <div>
                  ...
                  <strong>
                    {selected.max_shared_users_per_merchant ??
                      "—"}
                  </strong>
                </div>
              </div>
            ...
            <div className="drawer-footer">
              A suspicious network is an
              investigative candidate, not
